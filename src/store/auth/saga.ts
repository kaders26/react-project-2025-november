...
function* asyncLogin({
  ...
  try {
    ...
    // Rol bilgisini doğru şekilde kaydet: obje ise name veya id'yi al
    const roleValue = response.data!.role;
    if (typeof roleValue === 'object' && roleValue !== null) {
      AuthSession.setRoles(roleValue.name || String(roleValue.id));
    } else {
      AuthSession.setRoles(String(roleValue));
    }
